...
/*export const parseCondExp = (params: Sexp[]): Result<CondExp> => 
    ...
    !allT(isCompoundSExp, params) ? makeFailure("Invalid cond clauses"):
    
    params[params.length - 2][0] !== "else" ? makeFailure("Should end with else clause") :

    params.length < 2 ? makeFailure("Not enough parameters") :
    

    bind(mapResult(parseCondClause, params), (condClauses : CondClause[]) => makeOk(makeCondExp(condClauses)));
*/

export const parseCondExp = (params: Sexp[]): Result<CondExp> => 
    !allT(isCompoundSExp, params) ? makeFailure("Invalid cond clauses") :
    params[params.length - 1][0] !== "else" ? makeFailure("Should end with else clause") :
    params.length < 2 ? makeFailure("Not enough parameters") :
    bind(mapResult((param) => parseCondClause(params as Sexp[]), params), (condClauses : CondClause[]) => makeOk(makeCondExp(condClauses)));



export  const parseCondClause = (clause: Sexp[]): Result<CondClause> => 

        
        clause.length < 2 ? makeFailure("A cond clause must have at least two elements") :
        
        clause[0] === "else" ?  bind(mapResult(parseL31CExp, rest(clause)), (then) => makeOk(makeCondClause(makeStrExp("else"), then))):

        bind(parseL31CExp(clause[0]), (test) =>
            bind(mapResult(parseL31CExp, rest(clause)), (then) => makeOk(makeCondClause(test, then))));
...
const unparseCondClause = (exp: CondClause) : string =>
    isStrExp(exp.test) ?
    `(else ${map(unparseL31, exp.then)})` :
    `(${unparseL31(exp.test)} ${map(unparseL31, exp.then)})`;
   // `(${unparseL31(exp.test)} ${unparseL31(exp.then)} )`

const unparseCondExp = (exp: CondExp): string => {
    const clauses = exp.condclauses.map(unparseCondClause).join(" ");
    return `(cond ${clauses})`;
} 
